<?php
	...
	if(isset($subdomain[2]) && $subdomain[2]!= "")
	{
		$account->account= 'recent_articles';		
		//$account->logbook_name= $subdomain[2];
	...
	{
		//$account->FetchArticle($account->logbook_id, $account->article_id, 0, "");	
		include_once("./templates/index_mobile.php");
	}
	else if($account->chapter_name== "feed")
	{
		include_once("./templates/index_feed.php");
	}
	else if($account->chapter_name== "canvas")
	{
		...
		include_once("./templates/index_canvas.php");
	}
	else if($account->chapter_name== "wiki")
	{
		//echo $account->logbook_id;
		$account->FetchArticle($account->logbook_id, $account->article_id, 1, "");
		if($account->chapter_name== "fullscreen")
		{
			include_once("./templates/index_wiki.php");
		}
		else
		{
			include_once("./templates/index_standard.php");
		}
	}
	else if($account->chapter_name== "3d")
	{
		$account->chapter_id= 15;
		$account->chapter_name= "3d";
		//$account->FetchArticle($account->logbook_id, $account->article_id, 1, "");
		include_once("./templates/index_3d.php");
	}	
	else
	{
		$account->FetchArticle($account->logbook_id, $account->article_id, 1, "");
