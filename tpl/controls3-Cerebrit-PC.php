...
<div id="controls" page="3">
	...
	<div id="chapter_controls">
		
		<div id="chapter_select"></div>
		
		<div id="logbook_select"></div>
		
	</div>

	<div id="back2"><a href="javascript:">Back</a></div>

	<input type="button" class="btn" name="save" id="save_chapter" value= "S"/>	
...
<script>
	...
	AttachChapterLinkEvents();
	AttachLogbookLinkEvents();
	
	$("#save_chapter").click(function() {
	
		var chapter_arr= new Array();
		//var logbook_arr= new Array();
		var logbook_id= 0;
		var article_id= $("#logbooks").attr("article_id");
		
		$(".chapter_link").each(function() {
		
			if($(this).hasClass("selected"))
			{
				chapter_arr.push($(this).attr("chapter_id"));
			}
		});
		...
		$(".logbook_link").each(function() {
			...
			if($(this).hasClass("selected"))
			{
				logbook_id= $(this).attr("logbook_id");
			}
		});
		
		SaveChapter(article_id, chapter_arr, logbook_id);
	});	
